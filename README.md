...
- `activities.json` – data file for Oppgave 5, created by the program on first save (not part of the delivery)
...
A console program for planning and tracking activities. On start it tries to
load `activities.json`. On the first run the file does not exist yet: the
program says so and starts with an empty list. Example data can be loaded with
option 10 (from `data.py`), and the file is created the first time the user
saves (option 8 or the exit prompt). From then on it is loaded automatically
at start. The menu is shown again after every action.
...
The example data file required by the assignment is `data.py`
(`sample_activities`), loaded through option 10; `activities.json` is the
runtime file the program writes itself.
